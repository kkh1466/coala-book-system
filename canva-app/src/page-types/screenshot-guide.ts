...
import type { GuideStep, ScreenshotGuidePage } from "../types/book-spec";
 ...
 * 캡처를 합쳤다. 단계마다 흰 카드 하나이고, 카드 안에 "STEP n. 동작" 제목,
 * 지시문, 캡처 자리가 위에서 아래로 놓인다. 카드 사이에는 아래쪽 화살표가 있다.
 *
...
function stepCardItem(
  ...
  if (contentHeight > maxInner) {
    ...
    throw new PageContentTooDenseError(
      options.pageId,
      `단계 '${step.title}'의 글이 캡처와 함께 한 페이지에 들어가지 않습니다. 지시문을 줄이거나 단계를 나눠 주세요.`,
    );
  }
