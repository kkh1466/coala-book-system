...
- `--layout`을 주면 배치 엔진(`layoutBook`)까지 돌려 원고 페이지마다 Canva 몇 장이 되는지(`← 분할됨`), 이미지·순서도 자리가 어느 쪽에 몇 개인지 보고한다. Canva SDK는 글만 모으는 대역으로 바꿔 끼우고 글꼴은 기본 글꼴로 가정하므로 실제 생성과 1~2px 차이가 날 수 있다. 배치 단계의 원고 오류(카드에 들어가지 않는 글 등)도 여기서 잡힌다.
- 검사하지 않는 것: 내용의 정확성, 이미지 파일의 존재와 실제 비율.
...
- 단계별 진행(`step-process`): 단계 읽기, 내용 필수·이미지 금지·STEP 번호 기입 금지 검증과 행 번호(파서와 BookSpec 검증이 같은 판별 사용), 카드·화살표·번호 배치(`screenshot-guide`와 공용 `step-cards.ts`), 페이지 분할과 안전 영역, 한 카드에 들어가지 않는 글의 원고 오류
- 스크린샷 따라하기(`screenshot-guide`): 단계 읽기와 캡처 자리 필수·단일 검증의 행 번호, 캡처와 별개인 설명(문단·목록) 필수 검증(`alt`·`caption`은 설명이 아님, 파서와 BookSpec 검증이 같은 판별 함수 사용), STEP 번호 직접 기입 거절, 원본 치수의 흰 카드와 자동 번호 제목, 카드 안쪽 너비의 캡처 자리와 생성 후 보고, 카드 사이 화살표, 카드를 자르지 않는 페이지 분할과 안전 영역 준수, 세로 캡처의 비율 유지 축소, 한 카드에 들어가지 않는 글의 원고 오류
- 가로 흐름(```` ```flow ````): '제목 | 부제' 줄 읽기(파서·원고 검사·BookSpec 검증이 `flow-strip.ts` 하나를 공유), 카드 수 2~5·줄 형식·페이지 허용 검증과 행 번호, 원본 치수의 카드·간격·연결선, 제목 굵은 검정·부제 파란색·가운데 정렬·28pt, 카드 높이 통일, 앞뒤 간격, 통째로 넘김
- 코드 상자(```` ```python ```` 등)와 실행 결과: 펜스 읽기와 들여쓰기·탭 보존, 코드와 `output`·`role="result"` 이미지의 짝 읽기(파서·원고 검사·BookSpec 검증이 `code-result.ts` 하나를 공유), 결과 없음·고아 결과·사이의 본문·결과 둘·빈 output·output 안의 문법 거절과 행 번호, concept 기본 배치 외 거절, 코드 상자·파란 실행 결과 라벨·결과 상자의 순서와 간격, 출력의 고정폭·28pt·줄 간격 2와 색 구분 없음, 긴 코드·긴 출력의 줄 경계 분할과 줄 보존, 연속 라벨, GUI 결과 자리의 라벨·캡션·보고 목록 역할·다음 페이지 이동, 고정폭 글꼴 조회
...
- comparison 표 3열·본문 5행 제한과 카드 배치 2~4장 제한(파서와 BookSpec 검증 양쪽)
...
`process-steps.png` 그대로 글만으로 된 STEP 카드다. `../test-input/step-process.md`(원고 2페이지, 둘째 페이지는 단계 6개라 나뉜다)를 쓴다. 잘못된 원고는 `../test-input/invalid/process-*.md`(이미지, 내용 없는 단계, STEP 번호 직접 기입).

- [ ] 첫 페이지가 원본 76쪽과 같은 구성이다: 제목, 도입문, 흰 카드 세 장, 카드 사이 회색 아래쪽 화살표, 카드 안 "STEP 1." 제목과 문단·목록.
- [ ] 둘째 페이지에서 카드가 잘리지 않고 다음 장으로 넘어가며, 넘어간 카드 위에도 화살표가 있고 번호가 이어진다.
- [ ] 번호 목록(`1.` `2.`)이 카드 안에서 번호와 함께 보인다.

## 스크린샷 따라하기 수동 검증

자동 테스트는 카드·화살표·캡처 자리의 위치와 크기만 확인한다. 기본 원고는 `../test-input/screenshot-guide.md`(단계 4개, 반 너비 캡처라 한 페이지에 두 단계씩)이고, 상황별 원고를 한데 모은 `../test-input/guide-all.md`(원고 11페이지, 캡처 자리 33곳)를 한 번 올리면 아래를 모두 볼 수 있다. 자동 테스트가 이 원고를 배치해 안전 영역과 자리 수를 확인하지만, 실제 모습은 Canva에서 본다.

| `guide-all.md`의 페이지 id | 보는 것 |
| --- | --- |
| `ratios` | 16:9·4:3·3:2·1:1·3:4·9:16 캡처, 모두 반 너비. 카드 여러 장이 한 페이지에 놓이고 세로 캡처도 비율을 지킨다. |
| `widths` | `text`·`full`·`half`. 반 너비 카드 두 장이 한 페이지에 놓이고 사이에 화살표가 있다. |
| `captions` | 캡션, 캡처가 지시문 앞·뒤·문단과 목록 사이에 있는 순서 보존, 비율 미지정 경고, 긴 지시문과 번호 목록. |
| `many-steps` | 단계 10개, 모두 반 너비. 한 페이지에 두 단계씩 놓이고, 번호가 페이지를 넘어 이어지며, 넘어간 카드 위에 화살표가 있다. |
| `banner`, `long-intro`, `single` | 4:1 배너와 지면에 맞게 줄어드는 1:4 세로 띠, 긴 도입문 뒤의 첫 카드, 두 줄로 접히는 제목, 단계 하나뿐인 페이지. |
| `chapter-03-*` | chapter-opening → screenshot-guide → concept(Tip) → practice-checklist. 쪽번호와 이미지 자리 목록이 페이지 타입을 넘어 이어진다. |

`../test-input/invalid/code-*.md`는 코드·결과 규칙을 일부러 어긴 원고다: 결과 없는 코드, 고아 output, 고아 결과 이미지, 코드와 결과 사이의 본문, 결과 둘, 빈 output. 각각 오류 하나가 행 번호와 함께 나와야 한다.

`../test-input/invalid/guide-*.md`는 일부러 틀린 원고다. 앱에 올리면 생성 단추가 켜지지 않고 오류 목록이 보여야 한다: 캡처 없는 단계, 캡처가 둘인 단계, 설명 없이 캡처만 있는 단계, `STEP 1.`을 직접 적은 제목, 첫 단계 위의 캡처, 따라하기 안의 Tip, 범위를 벗어난 비율, 그리고 여러 오류가 한 번에 보고되는 원고.

- [ ] 단계마다 흰 카드 하나가 있고 제목이 `STEP 1.`부터 순서대로 붙어 있다. 제목은 30pt 굵게, 지시문은 28pt다.
- [ ] 카드 사이에 회색 아래쪽 화살표가 가운데 놓여 있다. 다음 페이지로 넘어간 카드 위에도 화살표가 있다.
- [ ] 캡처 자리가 카드 안쪽 너비로 카드 안에 놓여 있고, 이미지를 끌어다 놓으면 상자 크기 그대로 채워지며 카드와 지시문이 움직이지 않는다.
- [ ] 캡처가 지시문보다 위에 있는 3단계에서 순서가 원고와 같다.
- [ ] 생성이 끝나면 "이미지 자리 4곳"이 쪽번호와 함께 보고된다.
- [ ] 카드가 페이지 경계에서 잘리지 않고 쪽번호 영역을 덮지 않는다.

## 모든 상자 한 번에 보기

`../test-input/all-boxes.md`(원고 11페이지)는 앱이 그리는 상자 14종을 한 원고에 모은 것이다: 챕터 제목 테두리, 개념 카드 2열, 프롬프트·응답 상자, Tip, 코드·실행 결과 상자, 핵심정리, 가로 흐름, 일반 이미지 자리, GUI 결과 자리, 비교 표, 실습 카드, 실습 목표 체크리스트, 캡처 있는 STEP 카드, 캡처 없는 STEP 카드, 순서도 자리. 상자마다 글에 `→`를 섞어 두었다. 대체 글꼴로 그려지는 글자가 상자 오른쪽을 넘는지 한 번에 본다.

- [ ] 어떤 상자에서도 글이 오른쪽 테두리를 넘지 않는다. 넘는 상자가 있으면 그 상자에도 강조 박스와 같은 `textSlack` 여유를 적용한다.
- [ ] 이미지 자리 4곳(일반 1, 실행 결과 1, 캡처 2)과 순서도 자리 1곳이 보고된다.

## 가로 흐름 수동 검증

`caution-box.png`(59쪽) 위쪽의 카드 흐름이다. `../test-input/flow-strip.md`(원고 2페이지)를 쓴다. 첫 페이지는 원본 59쪽의 흐름·본문·핵심정리를 옮긴 것이라 나란히 비교할 수 있다(원본의 표는 concept 페이지에서 지원하지 않아 뺐다). 잘못된 원고는 `../test-input/invalid/flow-*.md`(카드 하나, 여섯, 줄 형식, 카드 배치).

- [ ] 흰 카드 네 장이 전체 단 너비에 같은 간격으로 놓이고, 카드 사이에 가는 연보라(`#8A8BBC`) 선이 세로 가운데에 있다.
- [ ] 카드 안 제목은 굵은 검정, 부제는 파란색이며 둘 다 28pt 가운데 정렬이다. 원본과 높이(약 183px)가 비슷하다.
- [ ] 둘째 페이지에서 부제 없는 카드 두 장, 카드 다섯 장, 제목이 두 줄로 접히는 카드가 각각 맞게 보이고, 접히는 흐름의 카드 높이가 모두 같다.
- [ ] 핵심정리 상자의 라벨이 "📑  핵심정리"(이모지와 글자 사이 두 칸)이고, `→`가 든 첫 줄을 포함해 어떤 줄도 상자 오른쪽을 넘지 않는다.

## 코드 상자 수동 검증

원본 교재에 코드 페이지 예시가 없어 모양은 앱이 정한 것이다. `../test-input/code-block.md`(원고 6페이지)를 쓴다. 기본 출력, 변수·조건문, 반복문, JavaScript, GUI 결과 이미지, 긴 코드 분할, 긴 출력 분할, 결과 이미지가 다음 페이지로 넘어가는 경우를 차례로 담고 있다.

- [ ] 텍스트 실행 결과가 코드 상자 아래 **별도의 회색 상자**에 있고, 두 상자 사이에 파란색(`#1800AD`) 굵은 "실행 결과" 문구가 상자 밖에 놓여 있다. 출력은 색 구분 없이 고정폭·28pt이고 앞쪽 공백(`  들여쓴 줄`)이 남아 있다.
- [ ] GUI 결과는 코드 상자 다음에 같은 파란 "실행 결과" 문구가 붙은 회색 자리로 놓이고, 이미지를 끌어다 놓아도 문구는 남는다. 생성 후 목록에 "· 실행 결과"가 표시된다.
- [ ] 긴 코드 페이지에서 다음 상자 라벨이 "Python (계속)"이다. 긴 출력 페이지에서 이어지는 결과 상자 위에 "실행 결과 (계속)" 문구가 있고 구구단 줄이 빠짐없이 순서대로다. 문구가 페이지 맨 아래에 홀로 남지 않는다.
- [ ] 마지막 페이지의 결과 이미지 자리가 다음 페이지 안전 영역 안에 놓인다.

- [ ] 옅은 회색(`#EEF1F5`) 둥근 상자 위에 "Python" 라벨이 회색으로, 그 아래 코드가 28pt·줄 간격 2로 보인다.
- [ ] 코드 글꼴이 고정폭이다. 생성 결과의 글꼴 보고에 코드 글꼴 이름이 있어야 하며, 계정에 고정폭 글꼴이 없으면 본문 글꼴로 그려진다.
- [ ] 한글 주석(`# 화면에 인사말을 보여 줍니다`)이 깨지지 않고 회색으로 보인다. 고정폭 글꼴에 한글이 없으면 Canva가 대체 글꼴로 그린다.
- [ ] 들여쓰기(스페이스 네 칸)가 Canva에서 그대로 보인다. 앞쪽 공백이 사라지면 `src/layout/code-block.ts`에서 공백을 줄바꿈 없는 공백(NBSP)으로 바꿔야 한다.
- [ ] `if`·`else`·`def`·`import` 같은 키워드가 파란색 굵게다.
- [ ] 두 번째 페이지의 상자 라벨이 "Python (계속)"이고 줄이 잘리지 않았다.
- [ ] 언어 이름 없는 블록의 라벨이 "코드"이고 색 구분이 없다.
